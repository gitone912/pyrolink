...
const ProductDetails = () => {
  ...
  const [createCart,cartResponse] = useCreateCartMutation();
  ...
  const addToCart = async () => {
    if (access_token) {
      const cartData = {
        user: userId, // Modify this line to get the user ID from your implementation
        item: id, // The current product ID
        quantity: 1, // Modify this line to get the quantity from user input
      };

      try {
        // Call the createCart mutation to add the product to the cart
        const response = await createCart(cartData)
        console.log(response); // Modify or handle the cart creation response as needed
        window.location.href = `/cart`; // Redirect to the cart page
      } catch (error) {
        console.error(error);
        // Handle error if cart creation fails
      }
    } else {
        ...
        <div className="relative mx-auto max-w-screen-xl px-4 py-8">
          <div className="grid grid-cols-1 items-start gap-8 md:grid-cols-2">
            <div className="grid grid-cols-2 gap-4 md:grid-cols-1">
              <img alt="Les Paul" src={currentData.image_link} className="aspect-square w-full rounded-xl object-cover" />
            ...
            <div className="sticky top-0">
              ...
              <div className="mt-8 flex justify-between">
                ...
                <p className="text-lg font-bold">${currentData.price}</p>
              </div>
              <div className="mt-4">
                <div className="prose max-w-none">
                  <p>
                    {currentData.technical_details}
                  </p>
                </div>
